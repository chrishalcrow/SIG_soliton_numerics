...
`play.ipynb` in the scripts folder. Overall, my project folder looks like this:
...
Ok. Let's use this function. We're going to use `play.ipynb`. This is a Jupyter notebook. To use it, we need to add another package to our enviroment. In the terminal, navigate to the project folder, type "uv add ipykernel" then press enter. Now open `play.ipynb` in VSCode.
