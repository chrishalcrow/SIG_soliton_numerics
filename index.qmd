...
If that worked: great! You're project is now _online_! If the last one error'd, that's ok. We'll get that sorted later.
...
class Grid():
    ...
    def __init__(
        ...
        self.grid_points = np.arange(-self.grid_length/2,self.grid_length/2, grid_spacing)
...
class Lagrangian():
    ...
    def __init__(
        self,
        V: Callable[[float], float], # Yup, in python you can pass functions are arguments. It's wild!
        ...
        if vacua is not None:
            for vacuum in vacua:
                ...
                assert np.isclose(dV(vacuum), 0), f"The given vacua do not satisfy dV({vacuum}) = 0"
```
...
class Soliton():
    ...
    def __init__(
        self,
        grid : Grid, # That's right: we can use our own classes as types! 
        lagrangian: Lagrangian,
        ...
        assert (initial_profile_function is None) or (initial_profile is None), "Please only specify `initial_profile_function` or `profile_function`"
    ...
    def compute_energy(self):
        ...
        energy = compute_energy_fast(self.lagrangian.V, self.profile, self.grid.num_grid_points, self.grid.grid_spacing)
        self.energy = energy
